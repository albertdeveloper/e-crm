    ...
    <app-layout>
        <template #header>
            ...
            <div class="float-right">
                ...
                <Inertia-link :href="route('admin.contacts.edit',{id:contact_data.id})" v-if="$page.allowed_gates.includes('contacts_process')"
                              class="cursor-pointer btn-primary mb-5 transition duration-300 ease-in-out focus:outline-none focus:shadow-outline bg-teal-700 hover:bg-teal-900 text-white font-normal py-2 px-4 mr-1 rounded">
                    Edit
                </Inertia-link>
        ...
        <note-modal :display="$page.modal" :data="contact_data"   :type="'contacts'"  />
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="p-6">

                        <p class="text-2xl">{{ contact_data.salutation }} {{ contact_data.name }} </p>
                        <p class="text-1xl mt-1"> - <b>{{ contact_data.account.name }}</b></p>
                        ...
                        <div class="flex mt-5">
                            ...
                            <div class="flex-1"><span class="ml-5">{{ contact_data.user.name }}</span></div>
                            <div class="flex-1"><b  class="float-right text-gray-400">Title: </b></div>
                            <div class="flex-1"><span class="ml-5">{{ contact_data.title }}</span></div>
                        </div>

                        <div class="flex mt-5">
                            <div class="flex-1 w-40"><b class="float-right text-gray-400">Email:</b></div>
                            <div class="flex-1"><span class="ml-5">{{ contact_data.email }}</span></div>
                            <div class="flex-1"><b  class="float-right text-gray-400">Department: </b></div>
                            <div class="flex-1"><span class="ml-5">{{ contact_data.department }}</span></div>
                        </div>

                        <div class="flex mt-5">
                            <div class="flex-1 w-40"><b class="float-right text-gray-400">Phone:</b></div>
                            <div class="flex-1"><span class="ml-5">{{ contact_data.phone }}</span></div>
                            <div class="flex-1"><b  class="float-right text-gray-400">Home phone: </b></div>
                            <div class="flex-1"><span class="ml-5">{{ contact_data.home_phone }}</span></div>
                        </div>

                        <div class="flex mt-5">
                            <div class="flex-1 w-40"><b class="float-right text-gray-400">Other phone:</b></div>
                            <div class="flex-1"><span class="ml-5">{{ contact_data.fax }}</span></div>
                            <div class="flex-1"><b  class="float-right text-gray-400">Mobile: </b></div>
                            <div class="flex-1"><span class="ml-5">{{ contact_data.date_of_birth }}</span></div>
                        </div>

                        <div class="flex mt-5">
                            <div class="flex-1 w-40"><b class="float-right text-gray-400">Assistant:</b></div>
                            <div class="flex-1"><span class="ml-5">{{ contact_data.assistant }}</span></div>
                            <div class="flex-1"><b  class="float-right text-gray-400">Assistant phone: </b></div>
                            <div class="flex-1"><span class="ml-5">{{ contact_data.assistant_phone }}</span></div>
                        </div>

                        <div class=" mt-5">
                            <p class="text-1xl"><b>Notes:</b> (<span class="text-sm">{{contact_data.notes.length}}</span>) </p>
                            <div v-for="note in contact_data.notes" v-if="contact_data.notes.length > 0" class="flex mt-3">
                                <div class="flex-1 w-1/3 bg-white px-4 py-4 rounded ml-15  shadow  ">
                                    <div v-if="note.user_id == $page.user.id">
                                        <div class="float-right" >
                                            <Inertia-link  preserve-scroll="1" preserve-state="1" method="DELETE" :href="route('admin.notes.destroy',{id:note.id})" class="focus:outline-none hover:text-red-500">
